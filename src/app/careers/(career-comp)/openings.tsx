...
const JobOpenings = () => {
  ...
  console.log(careers);
  return (
    <main className='z-[2] bg-primary w-screen h-fit  flex flex-col justify-between items-center gap-16 p-4 sm:py-[4rem] sm:p-[8rem]'>
      <div className='w-full h-fit flex flex-col sm:flex-row justify-center items-start gap-4 sm:gap-[8rem]'>
        ...
        <div className='w-full text-justify sm:w-3/5 h-fit flex flex-col justify-between items-start gap-[4rem]'>
          <p className='text-para'>
            <TextFormatter text={content?.description || ''} />
          </p>
          <div className='grid sm:grid-cols-2 grid-cols-1 w-full justify-start items-center gap-16'>
            {content?.items.map((item: any, idx: number) => (
              <div className='relative flex flex-col gap-1' key={idx}>
                <hr className='absolute -top-4 w-full h-[2px] bg-gradient-to-r from-accent1 to-accent2' />
